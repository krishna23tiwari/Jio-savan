...
const PunjabiSongs = () => {
  ...
  const handleAddToPlaylist = (song) => {
    const songExists = playlist.find((item) => item.id === song.id);
        if (songExists) {
    ...
    <div className={`min-h-screen p-6 transition-all ease-in ${darkMode ? "bg-white text-black" : "bg-gray-900 text-gray-100"}`}>
      ...
      <div className="flex flex-wrap justify-center gap-8">
        {search.map((song) => {
          ...
          const addToPlaylist = playlist.find((item) => item.id === song.id);

          return (
            <div 
              key={song.id} 
              className="relative w-full max-w-lg rounded-lg overflow-hidden cursor-pointer"
              onClick={() => setActiveSong(song.id === activeSong ? null : song.id)} 
            >
                ...
                <div className="mt-4 flex gap-4">
                  {addToPlaylist ? (
                    <button
                      ...
                      className="px-4 py-2 bg-blue-400 text-white rounded-lg hover:bg-blue-600 transition-all ease-in"
                    >
